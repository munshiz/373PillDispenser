...
void Home_Screen(void (*nextScreen)()) {
	...
	LCD_Font(120, 80, refill, _Open_Sans_Bold_20, 1, ILI9488_BLACK);

	fillRect(250, 40, 150, 75, ILI9488_RED);
	LCD_Font(300, 80, scan, _Open_Sans_Bold_20, 1, ILI9488_BLACK);

	fillRect(160, 125, 150, 75, ILI9488_RED);
	LCD_Font(190, 165, dispense, _Open_Sans_Bold_20, 1, ILI9488_BLACK);

	fillRect(70, 210, 150, 75, ILI9488_RED);
	LCD_Font(100, 250, new_pill, _Open_Sans_Bold_20, 1, ILI9488_BLACK);

	fillRect(250, 210, 150, 75, ILI9488_RED);
	LCD_Font(260, 250, fingerprint, _Open_Sans_Bold_20, 1, ILI9488_BLACK);
	...
	if (touchX >= 160 && touchX < 310 && touchY >= 125 && touchY < 200) {
...
void FingerPrint_Manager(void (*nextScreen)()) {

	//Setting the background to be white
	fillScreen(ILI9488_WHITE);

	// Setting X and Y coordinates to 0 by default 
	uint16_t touchX = 0, touchY = 0;

	// Needs just 2 buttons, 1 for Add fingerprint to set the fingerprint sensor into add fingerprint mode and then a done/back button 

}


void New_Pill(void (*nextScreen)()) {
